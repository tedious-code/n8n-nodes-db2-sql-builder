...
import { createPool, getConnectionString, queryAsync } from "../GenericFunctions";
...
export async function executeQueryAsync(
	...
	try {
		...
		for (let i = 0; i < queries.query.length; i++) {
			...
			const { sql, values, empty} = buildSqlAndBindings(
				q.sql,
				params,
			);
			...
			let raw;
			try {
				raw = await queryAsync(getConnectionString(credential), sql, values);
			} catch (e) {
					throw new Error(
						[
							'❌ SQL Execution Error',
